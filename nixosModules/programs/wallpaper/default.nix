...
let
  wallpaper-secret =
    if builtins.pathExists ../../../secrets/wallpaper-${config.networking.hostName}.png.age then
      "wallpaper-${config.networking.hostName}.png"
    else
      "wallpaper-fallback.png";

  wallpaper-secret-dark =
    if builtins.pathExists ../../../secrets/wallpaper-${config.networking.hostName}-dark.png.age then
      "wallpaper-${config.networking.hostName}-dark.png"
    else
      wallpaper-secret;
...
in
{
  ...
  config = lib.mkIf config.sophrams.wallpaper.enable {
    ...
    age.secrets =
      if wallpaper-secret == wallpaper-secret-dark then
        {
